...
import com.dianping.hawk.jmx.support.Ignore;
...
public class PigeonConfig {
	...
	private Boolean pigeon150EnabledSetByJmx;
	...
	private static final String LION_PIGEON150_ENABLED = "pigeon.150.enabled";
	...
	private static final boolean DEFAULT_PIGEON150_ENABLED = true;
	...
	private PigeonConfig() {
		...
		if (isHawkApiValid) {
			try {
				...
				try {
					logger.error("Register loadbalance manager to jmx failed.");
				} catch (Throwable e2) {e2.printStackTrace(System.err);}
			}
		}
	}
	...
	//1.5.0全局开关, 稳定后, 需要移除该开关, important!!!
	@Deprecated
	public static boolean isPigeon150Enabled() {
		Boolean isPigeon150Enabled = getInstance().pigeon150EnabledSetByJmx;
		if (isPigeon150Enabled == null && isLionApiValid()) {
			try {
				isPigeon150Enabled = ConfigCache.getInstance().getBooleanProperty(LION_PIGEON150_ENABLED);
			} catch (Throwable e) {
				logLionError(LION_PIGEON150_ENABLED);
			}
		}
		return isPigeon150Enabled != null ? isPigeon150Enabled : DEFAULT_PIGEON150_ENABLED;
	}
	...
	public static boolean isHeartBeatAutoPickOff() {
		return isPigeon150Enabled() && getBooleanValueFromLion(LION_HEARTBEAT_AUTOPICKOFF, DEFAULT_HEARTBEAT_AUTOPICKOFF);
	}
	...
	public void setPigeon150EnabledByJmx() {
		this.pigeon150EnabledSetByJmx = true;
	}
	
	public void setPigeon150DisabledByJmx() {
		this.pigeon150EnabledSetByJmx = false;
	}
	
	public void clearPigeon150EnabledByJmx() {
		this.pigeon150EnabledSetByJmx = null;
	}

...
import static com.dianping.dpsf.process.filter.InvocationProcessFilter.ProcessPhase;
...
public class RemoteInvocationHandlerFactory {
    ...
    private static Map<InvocationInvokeFilter.InvokePhase, List<InvocationInvokeFilter>>        internalInvokeFilters
            =   new LinkedHashMap<InvocationInvokeFilter.InvokePhase, List<InvocationInvokeFilter>>();
    private static Map<InvocationProcessFilter.ProcessPhase, List<InvocationProcessFilter>>     internalProcessFilters
            =   new LinkedHashMap<InvocationProcessFilter.ProcessPhase, List<InvocationProcessFilter>>();

    public static RemoteInvocationHandler createInvokeHandler(Map<InvocationInvokeFilter.InvokePhase, List<InvocationInvokeFilter>> filters) {
        return createHandler(internalInvokeFilters, filters);
    }

    public static RemoteInvocationHandler createProcessHandler(Map<InvocationProcessFilter.ProcessPhase, List<InvocationProcessFilter>> filters) {
        return createHandler(internalProcessFilters, filters);
    }

    private static <K, V extends RemoteInvocationFilter> RemoteInvocationHandler createHandler(Map<K, List<V>> internalFilters, Map<K, List<V>> filters) {
